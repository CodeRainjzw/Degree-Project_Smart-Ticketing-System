...
<%
    String a = null;
    String user = String.valueOf(request.getParameter("registration-username"));
    String password = String.valueOf(request.getParameter("registration-password"));
    String identity = String.valueOf(request.getParameter("registration-identity"));
...
<div id="wrapper" class="wrapper">
    ...
    <section class="section-padding-equal-70 bg-accent">
        <div class="container">
        <%if(user.equals("") || password.equals("") || identity.equals("")){
        		 a="Please fill out all blanks.";%>
        	 <div class="error-page-box-layout1">
        <h2 class="item-title"><%=a%></h2>
        <a href="register.jsp" class="error-btn">Register</a>
    </div>
       <%}else{ %>
        <%
            if (userDAO.CreateUser(user,password,identity)){
        a="Successfully Register, please log in.";%>
        <div class="error-page-box-layout1">
                <h2 class="item-title"><%=a%></h2>
                <a href="login.jsp" class="error-btn">Login</a>
            </div>
        <%}else{
        a="The username has already exist, please input another one.";%>       
            <div class="error-page-box-layout1">
                <h2 class="item-title"><%=a%></h2>
                <a href="register.jsp" class="error-btn">Back</a>
            </div>
            <%} 
            }%>
        </div>
