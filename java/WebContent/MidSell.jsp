...
<div id="wrapper" class="wrapper">
    ...
    int price = Integer.valueOf(request.getParameter("money"));
    
    if (price>=0){
    int hash = Integer.valueOf(request.getParameter("hash"));
    if(type.equals("ticket")){
    sellDAO.saveoriginalprice(u.getUname(),hash,sellDAO.getoriginalprice(u.getUname(),hash));
    sellDAO.savefirstprice(u.getUname(),hash,sellDAO.getoriginalprice(u.getUname(),hash));
    }
    sellDAO.defineprice(u.getUname(),hash,price);
    sellDAO.setnewinfo(u.getUname(),sellDAO.changeInfo(u.getUname(),hash,price),hash);
    sellDAO.setselling(u.getUname(),hash);
    try {
        userDAO.userconfirmsell(u.getUname(),hash);
    } catch (Exception e) {
        e.printStackTrace();
    }
...
<%=hash%>
<%}else{%>
			<h2 class="item-title">Resale amount can not be negative</h2>
                	<a href="ticket.jsp" class="error-btn">Back</a>
                	<%} %>
                	</div>
        </div>
    </section>
