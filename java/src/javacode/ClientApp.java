...
public class ClientApp {
	...
	public static String main(String[] command) throws Exception {
		...
		Path walletPath = Paths.get("G:\\JZW\\STUDY\\2022_Spring\\Project\\DPworkspace\\java\\wallet");
		Wallet wallet = Wallets.newFileSystemWallet(walletPath);
		// load a CCP
		Path networkConfigPath = Paths.get("G:\\JZW\\STUDY\\2022_Spring\\Project\\DPworkspace\\java\\connection.json");
		...
		try (Gateway gateway = builder.connect()) {
			...
			result = contract.submitTransaction("changeTicketOwner",command[1], command[2], command[3]);
			}
			else if(command[0].contentEquals("deleteTicket")) {
				result = contract.submitTransaction("deleteTicket",command[1]);
			}
